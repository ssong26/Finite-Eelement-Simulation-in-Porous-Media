...
coeff = freq*2*pi*sqrt(effective_density/effective_modulus);
...
figure;
plot(x_temp(number_length:(-1):1),real(pressure(1:number_height:nnode)),'LineWidth',4);hold on
scatter(x_theory,real(y_theory),300)
legend('numerical simulation','theoretical prediction')
xlabel('X-direction');ylabel('Real-Pressure')
% imag term
figure
plot(x_temp(number_length:(-1):1),imag(pressure(1:number_height:nnode)),'LineWidth',4);hold on
scatter(x_theory,imag(y_theory),300)
legend('numerical simulation','theoretical prediction')
xlabel('X-direction');ylabel('Imag-Pressure')
%===================================end====================================
end
